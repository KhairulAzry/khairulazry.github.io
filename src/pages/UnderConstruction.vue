<template>
    <div class="flex w-full h-screen bg-dark">
      <div class="flex-1 flex flex-wrap items-center justify-center">
        <div class="space-y-2 text-white text-center">
          <div class="text-h3 md:text-h1 font-black uppercase">Page Under Construction</div>
          <div>This page is under construction. <br/> Thank you for visiting.</div>
          <div class="mt-2"><router-link to="/" class="px-4 py-1 bg-white border rounded-full text-dark">Go to Home</router-link></div>
        </div>
      </div>
    </div>
  </template>
  
  <script>
  export default {
      name: 'Under Construction'
  }
  </script>